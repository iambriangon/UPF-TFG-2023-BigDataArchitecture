...
spark = SparkSession \
    ...
    .appName("Spotify Historical Upload") \
    .enableHiveSupport() \
    .getOrCreate()
...
schema = StructType() \
      ...
      .add("peak_rank", IntegerType(), True) \
      .add("previous_rank", IntegerType(), True) \
      .add("days_on_chart", IntegerType(), True) \
      .add("streams", IntegerType(), True) \
...
df_hive = df.select('rank', 'artist_names', 'track_name', 'source', 'date')
